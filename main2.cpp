...
int main(int argv, char** argc)
{
    ...
    */
    ...
    int sz = (BlockX+2)*(BlockY+2)*sizeof(double);
    double *ArrayOld = (double*)malloc((BlockX+2)*(BlockY+2)*sizeof(double));
    ...
    for (int i=0; i<size+2; i++) {
      ...
      MPI_Scatter(Arrayinit, size*size+2, MPI_DOUBLE, ArrayOld, ((BlockX+2)*(BlockY+2)), MPI_DOUBLE, 0, MPI_COMM_WORLD);
   // printArray(ArrayOld, BlockX, BlockY);
   
        

    double *ArrayNew = (double*)malloc((BlockX+2)*(BlockY+2)*sizeof(double));
    double *extra  = (double*)malloc((BlockX+2)*(BlockY+2)*sizeof(double));
   ...
   for(int it = 0; it<iterations; ++it)
   {
    ...
    double *sbuf =  (double*)malloc((BlockX+2)*(BlockY+2)*sizeof(double)); // send buffer (west, east, north, south)
    double *rbuf =  (double*)malloc((BlockX+2)*(BlockY+2)*sizeof(double)); // receive buffer (w, e, n, s)
   ...
   //update 
    ...
    // std::cout << "Before Gather: " << rankX << "," << rankY << " RR: " << rank << std::endl;
     double *recvArray =  (double*)malloc( (BlockX+2)*(BlockY+2)*sizeof(double));
    int err = MPI_Igather(ArrayOld, szs, MPI_DOUBLE, recvArray, szs, MPI_DOUBLE, 0, cartesionCom, &req2); //because catesian
...
//DEF: MPI_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
   ...
   //skip for high iterations (on cluster)
